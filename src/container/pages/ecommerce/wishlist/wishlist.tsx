  ...
  const myfunction = (InputData: string) => {
    ...
    for (allElement of allData) {
      ...
      if (allElement.title.toLowerCase().includes(InputData.toLowerCase())) {
        if (
          allElement.title.toLowerCase().startsWith(InputData.toLowerCase())
        ) {
        ...
        <Col xl={12}>
          <Card className="custom-card">
            <Card.Body className="card-body d-sm-flex align-items-center justify-content-between">
              <div className="fs-15 mb-sm-0 mb-2">
                Total <span className="badge bg-success">12</span> products are
                wishlisted
              </div>
              ...
              <Col xxl={3} xl={6} lg={6} md={6} sm={12} key={Math.random()}>
                <Card className="custom-card product-card">
                  <Card.Body>
                    ...
                    <p className="product-name fw-semibold mb-0 d-flex align-items-center justify-content-between">
                      ...
                      <span className="float-end text-warning fs-12">
                        4.2<i className="ri-star-s-fill align-middle ms-1"></i>
                      </span>
                    </p>
                    ...
                    <p className="mb-1 fw-semibold fs-16 d-flex align-items-center justify-content-between">
                      <span>
                        {idx.newpr}
                        <span className="text-muted text-decoration-line-through ms-1 op-6 d-inline-block">
                          {idx.oldpr}
                        </span>
                      </span>
                      <span className="badge bg-secondary-transparent float-end fs-10">
                        72% off
                      </span>
                    </p>
                    <p className="fs-11 text-success fw-semibold mb-0 d-flex align-items-center">
                      <i className="ti ti-discount-2 fs-16 me-1"></i>Offer Price{" "}
                      {idx.offerprice}
                    </p>
                  </Card.Body>
                  <Card.Footer className="text-center">
                    ...
                    >
                      <i className="ri-shopping-cart-2-line me-2 align-middle d-inline-block"></i>
                      Move To Cart
                    </Link>
                    <Link
                      to={`${
                    ...
                    >
                      <i className="ri-eye-line me-2 align-middle d-inline-block"></i>
                      View Product
                    </Link>
                  </Card.Footer>
                </Card>
